...
if __name__ == "__main__":
    ...
    sns.displot(
        df_weather.query("log_demand > 0"), x="tempC", y="log_demand", kind="kde"
    )
    # Higher temperatures seem to slightly correlate with higher demand as expected

    sns.displot(
        df_weather.query("log_demand > 0"), x="precipMM", y="log_demand", kind="kde"
    )
    # In general higher precipitation can mean low demand

    # ------------------------------------------------------- #
    # Check if connectivity affects demand
    connectivity_related_variables = [
        "cycleway_connectivity",
        "tertiary_connectivity",
        "secondary_connectivity",
        "station_count",
    ]

    df_connectivity = df_train[connectivity_related_variables + [target]]
    df_connectivity["log_demand"] = df_connectivity[target].apply(
        lambda x: np.log(x + 1)
    )
    ...
    datetime_df = (
        df_train[["hour", "day_of_week", target]]
